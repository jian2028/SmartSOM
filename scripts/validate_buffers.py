"""Bounded item-9 acceptance: configured runs and both exact replay routes.
...
def check_run(resolved, *, expected=None):
    ...
    assert (
        replay_schedule(
            ...
            replace(
                ...
                arrivals=tuple(reversed(stored.arrivals)),
                transfers=tuple(reversed(stored.transfers)),
                action_order=tuple(reversed(stored.action_order)),
    ...
    return {
        ...
        "delivered_jobs": sum(
            t.destination.kind == "output"
            for t in (*simulation.transport_schedule, *simulation.transfer_schedule)
        ),
    }
...
def main():
    ...
    for case, expected in (
        ("buffers_direct_zero", 6),
        ("buffers_vehicle_zero", 8),
        ("buffers_post_one", 6),
    ):
    ...
    base = yaml.safe_load(
        (ROOT / "configs/scenarios/buffers_combined.yaml").read_text()
    )
    ...
    for agv, ja, mb, upt in itertools.product((False, True), repeat=4):
        for trigger in (
            ("dispatch_available", "arrival_event") if ja else ("dispatch_available",)
        ):
            for policy in ("spt_buffers", "first_feasible"):
                name = f"AGV{int(agv)}-JA{int(ja)}-MB{int(mb)}-UPT{int(upt)}-{trigger}-{policy}"
                scenario = {
                    **base,
